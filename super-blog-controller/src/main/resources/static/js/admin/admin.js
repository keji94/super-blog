...
var menuItem = Vue.extend({
    ...
    template:[
        ...
        '		<cite>{{item.resourceName}}</cite>',
        '		<i class="iconfont nav_right">&#xe697;</i>',
        '	</a>',
        '	<ul v-if="item.type === 0" class="sub-menu">',
        '		<menu-item :item="item" v-for="item in item.subResource"></menu-item>',
        '	</ul>',
        '	<a v-if="item.type === 1" :_href="item.resourceUrl"><i v-if="item.icon != null" :class="item.icon"></i><i v-else class="fa fa-circle-o"></i> <cite>{{item.resourceName}}</cite></a>',
        '</li>'
    ].join('')
});
...
var vm = new Vue({
    ...
    methods: {
        getMenuList: function () {
            $.getJSON("/admin/system/showNav?_"+$.now(), function(r){
                vm.menuList = r.data;
            });
        },
        getUser: function(){
